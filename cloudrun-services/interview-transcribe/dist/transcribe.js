...
async function transcribeAudio(options) {
    ...
    if (isVideoFile && isMP4File) {
        ...
        try {
            audioGcsUri = await (0, audio_extractor_1.extractAudioFromMP4)(gcsUri);
    ...
    for (let patternIndex = 0; patternIndex < configPatterns.length; patternIndex++) {
        ...
        console.log(`[TRANSCRIBE] Trying config pattern ${patternIndex + 1}/${configPatterns.length}:`, JSON.stringify(currentConfig, null, 2));
        if (isVideoFile || isLargeFile) {
            ...
            try {
                ...
                console.log('[TRANSCRIBE] Preview:', transcriptionText.substring(0, 100) + (transcriptionText.length > 100 ? '...' : ''));
