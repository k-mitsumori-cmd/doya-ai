...
for(const id of ['top','banner'])for(const [device,width,height]of [['desktop',1440,1000],['mobile',390,844]]){
  ...
  if(!await page.$('[data-operation-demo="2026-09-08"]'))throw Error('Motion release missing');
  await page.waitForSelector('[data-demo-ready="true"]');await page.evaluate(()=>document.fonts.ready);await page.reload({waitUntil:'domcontentloaded'});await page.waitForSelector('[data-demo-ready="true"]');
  ...
  await new Promise(r=>setTimeout(r,14000));await recorder.stop();execFileSync('/opt/homebrew/bin/ffmpeg',['-y','-i',path.join(temp,name),'-c','copy',path.join(out,name)],{stdio:'ignore'});const durationSeconds=Number(execFileSync('/opt/homebrew/bin/ffprobe',['-v','error','-show_entries','format=duration','-of','default=noprint_wrappers=1:nokey=1',path.join(out,name)],{encoding:'utf8'}));records.push({id,device,durationSeconds,viewport:[width,height],url:page.url(),file:name,bytes:fs.statSync(path.join(out,name)).size});console.log(name,records.at(-1).bytes);
