...
(async()=>{await Promise.all(Array.from({length:1},async()=>{
 ...
 try{while(cursor<ids.length){const id=ids[cursor++];for(const [device,w,h]of [['desktop',1440,1000],['mobile',390,844]]){
  ...
  if(resume&&fs.existsSync(path.join(dest,stem+'-record.json'))){const previous=JSON.parse(fs.readFileSync(path.join(dest,stem+'-record.json')));if(previous.status==='captured'&&previous.operation_demo===true){entries.push(previous);continue;}}
  try{
   ...
   await page.waitForSelector('[data-operation-demo="2026-09-08"]',{timeout:15000});await page.evaluate(()=>document.fonts.ready);await page.waitForSelector('[data-demo-ready="true"]');rec.operation_demo=true;await page.$$eval('.doya-renewal img',els=>els.filter(i=>!i.closest('dialog')).forEach(i=>i.loading='eager'));await wait(1800);
   ...
   await page.click('.doya-motion-toggle[aria-pressed="false"]');await wait(700);
