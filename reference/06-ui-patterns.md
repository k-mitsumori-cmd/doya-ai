...
| ブランドカラー | `#7f19e6` | インタビューサイドバー等で使用 |
...
ドヤマーケAI の「ポップさ」を構成する要素のまとめ。新サービス開発時もこの原則に従う。
...
- CTA ボタンにグロー効果（shadow-glow-blue）
- アクセントに Emoji を大きく使う（text-3xl 以上）
- 成功時にキャラクターアニメーション

❌ やらない:
- 全面的に暗い配色（ダーク基調は LP/Movie/Voice のサイドバーのみ）
- 彩度の低いグレー系だけでページを構成
- テキストだけの無機質な画面
```
...
バッジ: rounded-full (pill)
アイコン背景: rounded-lg (8px) 〜 rounded-xl (12px)
```

### 14.3 アニメーションの原則

```
登場: fade-in-up (下から浮き上がり) が基本
モーダル: scale 0.85→1 + backdrop-blur
ホバー: translateY(-2px) のリフト効果
キャラクター: float（ふわふわ）で常にアイドル状態
ナビアクティブ: layoutId でスムーズ切り替え
```
...
```
✅ やる:
- ダッシュボードの空状態に hello キャラ
- 生成中に working キャラ + 吹き出し
- エラー画面に error キャラ
- アップセルモーダルに present キャラ

❌ やらない:
- 全画面にキャラクターを置く（鬱陶しい）
- キャラクターでエラーメッセージの本質を隠す
- 複数キャラクターを同時に表示
```
...
- [ ] テキストだけでなく、Emoji やアイコンで視覚的なアクセントがある
- [ ] ホバーエフェクトがある（lift / glow / scale のいずれか）
- [ ] 登場アニメーションがある（fade-in-up / scale-in のいずれか）
- [ ] 角丸が適切（rounded-xl 以上）
- [ ] 空状態にキャラクターまたはイラストがある
- [ ] CTA ボタンがグラデーション + グロー
...
#### 16.2 ドヤ勤怠（kintai）が ToolSwitcherMenu に表示されない

`kintai` は `services.ts` で `status: 'active'` だが、`ToolSwitcherMenu.tsx` の `SERVICE_ICON_MAP` に **未登録**。
`getActiveServices()` は返すが、アイコンマッピングがないため `null` が返り、メニューに表示されない。

**対応**: `src/components/ToolSwitcherMenu.tsx` の `SERVICE_ICON_MAP` に `kintai` を追加。

### 🟡 Warning — 中優先度

#### 16.3 ドヤ展開AI（tenkai）の ToolSwitcherMenu 未登録

`tenkai` も `SERVICE_ICON_MAP` に未登録。現在は `coming_soon` なので影響なしだが、`active` に変更する際にメニューに表示されない。

**対応**: `active` への変更前に `SERVICE_ICON_MAP` への追加が必要。

#### 16.4 ドヤ勤怠（kintai）が 10-service-status.md に未記載

`kintai` サービスはステータス管理ドキュメントに記載がない。
