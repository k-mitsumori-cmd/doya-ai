...
| ID | 内容 | 規模 | 優先 | 状態 |
|---|---|---|---|---|
| R6-A | 正方形ロゴ4点の横長化 | 4点 | P0 | 依頼書あり（`round5`）・配色判断待ち |
| R6-B | 空状態の展開 | 10画面 | P1 | 素材・部品は完成済み。配線のみ |
| R6-C | 絵文字の置換 | 131ファイル / 508箇所 | P1 | `icons.tsx` は完成済み。置換が未着手 |
| R6-D | 旧ブランド紫の置換 | 503箇所 | P2 | — |
| R6-E | ユースケース シーン画像 | 34点 | P2 | 未着手 |
...
| 選択肢 | 内容 |
|---|---|
| A | 配色はそのまま。寸法だけ直す。作業量小・リスク低 |
| B | ブランド青へ寄せる。並びは揃うが2サービスの印象が変わる |

**この1点が決まらないと R6-A は着手できない。**なお `persona` の紫は
旧ブランド `#7f19e6` に近く、R6-D の置換方針とは逆行している。
...
| 判断 | 現状 `UseCases` は箇条書きで成立している。**画像が本当に要るかを先に見極める**（R6-B・R6-C を終えてから判断で構わない） |
...
| ID | 内容 |
|---|---|
| R6-A | `interview` / `persona` の配色をブランド青へ寄せるか（A: 現状維持 / B: 青へ） |
| R6-E | ユースケース画像を作るか、箇条書きのままにするか |
