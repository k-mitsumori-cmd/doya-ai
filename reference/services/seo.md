...
| ゲスト | 生成不可（ログイン必須） | — | ¥0 |
| 無料会員 | 3回/月 | 10,000字 | ¥0 |
| LIGHT | 10回/月 | 15,000字 | ¥2,980 |
| PRO | 30回/月 | 20,000字 | ¥9,980 |
| Enterprise | 200回/月 | 50,000字 | ¥49,800 |
...
### 画像生成
画像の生成・再生成はLIGHT以上（初回ログイン後1時間のお試し中も可）。サーバーは全画像生成APIで同じ権限を判定する。外部画像APIの保護上限は日本時間の1日100枚分で、一括生成は開始前に予定枚数を確保する。この上限は記事の月間作成枠とは別。`ensure` は同一記事への同時生成を拒否し、既存画像がある場合は不足分だけ生成する。

本番の生成画像は非公開のSupabase Storageバケット `seo-generated-images` に保存し、画像APIで所有者を確認して配信する。開発環境のローカル保存形式と旧レコードの読み取りは維持するが、旧本番画像が一時領域 `/tmp` から既に消えている場合は復元できない。バケットの設定・保存・再取得が失敗した場合は成功扱いしない。

記事削除時は画像の永続保存パスをDBトランザクション内の削除待ち台帳に記録する。削除後すぐに保存先を清掃し、ストレージ障害時は台帳を残して時間ごとのcronで再試行する。

生成画像はストレージへの書き込み前にもDBへ保留記録を残す。時間ごとのcronは1時間以上経った保留記録を照合し、画像DBに登録済みなら保留記録のみ削除、未登録なら保存画像を削除する。DB・ストレージ障害時は保留記録を残して再試行する。
...
最新50件で打ち切らず、作成日時/id降順で続き取得。タイトル/キーワード部分一致と状態検索はサーバーで全所有記事に適用。合計/生成中/完成は全所有記事の集計、検索一致数と表示数を別に表示。ゲストは未移管の自分の記事のみ。続き失敗は現在の一覧を保持して再試行、検索変更時は旧応答を破棄する。詳細はdocs/audits/2026-09-20-loop-265/report.md。
