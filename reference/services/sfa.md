...
> **2026-06-12 パイプライン中心の改修（実装済み）**:
> - **商談詳細モーダル**: カンバンのカードクリックで開き、商談名/金額/確度/取引先/**先方担当者名(contactName)**/商談日/完了予定日/**メモ(note)** を編集（`SfaDeal` に contactName・note 列を追加。本番DBは手動ALTER適用済み）
> - **担当者（contacts）・リード（leads）・活動（activities）はサイドバーから廃止**。担当者名は商談詳細のフリーテキスト、リードは商談パイプラインに集約、活動はタスクページに統合（メニュー名「タスク・活動」）。旧ページはURL直アクセスで残存
> - **タスクの商談連携**: `SfaTask.dealId` を API/UI で使用。カンバンカードに未完了タスク（最大3件+件数）をチェックボックス付きで表示、期限切れは赤。タスクページは商談名チップ+期日インライン編集（PATCHが dueDate/title 編集対応）
> - **AI次アクション**: 提案(次の一手/理由/リスク)に加えて**タスク候補2〜4件（期日付き）**を返し、モーダルでチェック・期日編集して一括追加（`suggestNextAction` が tasks[] を返す）
> - **活動タイムライン**: 商談詳細モーダル内にタスクと同じ操作感（追加フォーム+リスト）で表示・追加（既存 /api/sfa/activities の dealId 絞り込みを使用）
