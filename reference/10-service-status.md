...
| ~~ドヤコピーAI~~ | ~~`/copy`~~ | **廃止** | ✅ | ✅ 12頁 | ✅ 11件 | ✅ 3モデル | ✅ 統一プラン | ✅ |
| ~~ドヤワイヤーフレームAI~~ | ~~`/lp`~~ | **廃止** | ✅ | ✅ 9頁 | ✅ 9件 | ✅ 2モデル | ✅ 統一プラン | ✅ |
| ~~ドヤ広告シミュレーションAI~~ | ~~`/adsim`~~ | **廃止** | ✅ | ✅ 6頁 | ✅ 9件 | ✅ 1モデル | ✅ 統一プラン | ✅ |
| ~~ドヤ展開AI~~ | ~~`/tenkai`~~ | **廃止** | ✅ | ✅ 10頁 | ✅ 23件 | ✅ 6モデル | ✅ 統一プラン | ✅ |
| ドヤペルソナAI | `/persona` | active | ✅ | ✅ 5頁 | ✅ 3件 | ⚠️汎用 | ✅ 統一プラン | ✅ |
| ~~ドヤボイスAI~~ | ~~`/voice`~~ | **廃止** | ✅ | ✅ 10頁 | ✅ 12件 | ✅ 2モデル | ✅ 統一プラン | ❌ |
| ~~ドヤムービーAI~~ | ~~`/movie`~~ | **廃止** | ✅ | ✅ 11頁 | ✅ 15件 | ✅ 3モデル | ✅ 統一プラン | ❌ |
...
⚠️ **公開状態の正本は `src/lib/services.ts` の `status` と `HIDDEN_SERVICE_IDS`。**
この表は手で更新するため古くなる。食い違ったらコード側を信じること。

## 廃止したサービス

### 2026-08-17 に畳んだサービス（11件）

第3陣（カンタンマーケAI / ドヤWeb診断AI / ドヤロゴ / ドヤオープニングAI）:

| サービス | パス | 移行先 | 本番データ |
|---|---|---|---|
| カンタンマーケAI | `/kantan` | `/seo` | 専用テーブル無し |
| ドヤWeb診断AI | `/shindan` | `/` | 専用テーブル無し |
| ドヤロゴ | `/logo` | `/` | 専用テーブル無し |
| ドヤオープニングAI | `/opening` | `/` | プロジェクト3件・アニメ12件 |

⚠️ `/api/shindan/generate` は有料モデルを呼ぶ。一覧から隠れていただけで
直接叩けば動く状態が続いていた。
...
|---|---|---|
| ドヤムービーAI | `/movie` | 0件 |
| ドヤボイスAI | `/voice` | プロジェクト2件 |
| ドヤ展開AI | `/tenkai` | 0件 |
| ドヤコピーAI | `/copy` | 0件 |
| ドヤヒヤリングAI | `/interviewx` | プロジェクト3件・回答1件 |
| ドヤワイヤーフレームAI | `/lp` | プロジェクト1件 |
| ドヤ広告シミュレーションAI | `/adsim` | プロジェクト4件 |

いずれも後継サービスは無し。ポータルトップへ集約した。**データは消していない。**

⚠️ `/interviewx` には回答者向けの公開URL（`/interviewx/respond/[token]`）があり、
実際に回答が1件記録されている。畳んだ時点で、出回っている回答URLがあれば
使えなくなる。復活させる場合はそこも合わせて確認すること。

⚠️ `/interviewx` は現役の `/interview`（ドヤインタビュー）とは**別サービス**。
混同して巻き込まないこと。

### ドヤムービーAI（`/movie`）/ ドヤボイスAI（`/voice`）— 実施内容
