...
async function scenario({ nowMs, status = 'ended', owner = 'u', hasContent = true, acceptIncomplete = false, missingAudio = true, claimedAt = null }) {
  let providerCalls = 0
  let usageReads = 0
  let current = {
  ...
  const lease = { userId: owner, stoppedAt: new Date(10000), expiresAt: new Date(70000) }
  const model = {
  ...
  const api = load('src/app/api/cunning/sessions/[id]/report/route.ts', {
    ...
    '@/lib/cunning/recording-ledger': { readCunningRecordingUsage: async () => { usageReads++ } },
  })
