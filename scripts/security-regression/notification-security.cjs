...
async function main(){
  ...
  for(const[name,args,count]of[
    ...
    assert.equal(logs.length,1);assert.equal(notifications.length,count);assert.equal(diagnostics.length,count);assert(!JSON.stringify(notifications).includes('private-canary'));assert(!JSON.stringify(diagnostics).includes('private-canary'));if(count){const d=diagnostics[0][1];assert.match(d.incidentId,/^[0-9a-f-]{36}$/);assert.equal(d.argumentCount,args.length);assert.equal(d.firstArgKind,args[0] instanceof Error?'error':typeof args[0]);assert(['non-string','empty','1-32','33-128','129-512','513+'].includes(d.firstArgLength));assert(Array.isArray(d.frameKinds)&&d.frameKinds.length<=8);assert(d.frameKinds.every(kind=>['node-internal','prisma','next','vercel','bundler','anonymous','app-bundle','other'].includes(kind)));assert.equal(d.family,name==='next-auth family'?'next-auth':name==='warning with separate error'?'warning':'unclassified');assert(['missing','unparsed','parsed'].includes(d.stackState));assert(Number.isInteger(d.stackLineCount)&&d.stackLineCount>=0&&d.stackLineCount<=20);assert.equal(typeof d.stackHasLocation,'boolean');assert.equal(typeof d.stackExternalLocation,'boolean');assert.equal(typeof d.stackScriptLocation,'boolean');assert.equal(typeof d.stackExternalScriptLocation,'boolean');assert(Number.isInteger(d.stackTraceLimit)&&d.stackTraceLimit>=-1&&d.stackTraceLimit<=100);assert(notifications[0].includes(d.incidentId));assert.equal(d.errorTypes.length,args.filter(x=>x instanceof Error).length);}pass(name);
  }
  ...
  assert.equal(trace.exports.safeRuntimeSource('Error\n    at console.error (virtual-frame)\n    at handler (/var/task/chunks/12345.js?bundle=1:18:72)'), '12345.js:18:72');
  assert.equal(trace.exports.safeRuntimeSource('Error\n    at console.error (virtual-frame)\n    at handler [compiled:12345.js:18:72]'), '12345.js:18:72');
