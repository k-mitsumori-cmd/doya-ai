...
for(const mode of ['open','won','lost','default-won','no-stages','foreign','missing']) {
 ...
 const deps={'next/server':{NextResponse:Response},'@/lib/prisma':{prisma:{sfaStage:{findUnique:async()=>stage,findFirst:async()=>stage},sfaDeal:{create:async({data})=>{saved=data;return {id:'d',...data};}}}},'@/lib/sfa/access':{getSfaContext:async()=>({organizationId:'o',memberId:'m',userId:'u'}),orgSlugFrom:()=> 'org'},'@/lib/sfa/format':{bigIntToNumber:o=>JSON.parse(JSON.stringify(o,(_,v)=>typeof v==='bigint'?Number(v):v))},'@/lib/service-usage':{recordServiceUsage:async()=>{usage++;}}};
