...
(async()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),'doya-storage-audit-'));try{
 const lib=load('seo/lib/storage.ts',{'node:fs':fs,'node:path':path,'node:crypto':crypto,'@supabase/supabase-js':{},'@/lib/prisma':{prisma:{}}},{process:{cwd:()=>dir,env:{SEO_STORAGE_DIR:dir}}});
