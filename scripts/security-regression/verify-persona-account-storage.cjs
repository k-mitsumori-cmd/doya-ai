...
(async()=>{for(const kind of ['handleGenerate','handleModify'])for(const s of ['normal','unmounted','late-error','full','retry','changed-input'])await run(kind,s);await run('handleModify','replaced')})().catch(e=>{console.error(e);process.exitCode=1});
