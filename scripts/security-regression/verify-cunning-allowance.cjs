...
await check('only free exhausted recording quota can be expanded by an upgrade',async()=>{for(const tier of ['FREE','LIGHT','PRO']){serverUsage={...serverUsage,tier,limits:{tier,maxMinutesPerMonth:tier==='FREE'?60:1200,maxKnowledgeBases:1},remainingSeconds:0};const result=await integrated.canStartSession('u');assert.equal(result.code,'LIMIT');assert.equal(result.upgradeAvailable,tier==='FREE')}});
