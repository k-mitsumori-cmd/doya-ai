...
function harness(){const exports={};let user='owner',exists=true,queries=0,renders=0;class NextResponse{constructor(body,opts){this.body=body;this.status=opts?.status||200;this.headers=opts?.headers||{}}static json(body,opts){return new NextResponse(body,opts)}}
vm.runInNewContext(compiled,{exports,URL,Buffer,Uint8Array,console:{error(){}},require(name){
 ...
 if(name==='@/lib/prisma')return{prisma:{generation:{findFirst:async({where})=>{queries++;return exists&&where.userId==='owner'&&where.id==='known-generation'?{output:'data:image/png;base64,ZmFrZQ=='}:null}}}};
...
return{...exports,setUser(v){user=v},delete(){exists=false},get queries(){return queries},get renders(){return renders},req(etag){return{url:'https://example.test/api/banner/history/thumb?id=known-generation&w=320',headers:{get:name=>name==='if-none-match'?etag:null}}}};}
(async()=>{
