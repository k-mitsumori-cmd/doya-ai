...
function fixture(service, { deliverySuccess = true, role = 'owner' } = {}) {
  let creations = 0;
  let emails = 0;
  ...
  return { post: (body) => api.POST({ json: async () => body }), get creations() { return creations; }, get emails() { return emails; } };
}
