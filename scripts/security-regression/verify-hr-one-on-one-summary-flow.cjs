...
(async()=>{
 ...
 for(const endpoint of ['edit','ai'])for(const mode of ['completed','race','normal'])await check(endpoint+' '+mode,async()=>{
  ...
  assert.equal(response.status,mode==='normal'?200:409);assert.equal(writes,mode==='normal'?1:0);if(mode==='completed')assert.equal(calls,0);
 });
