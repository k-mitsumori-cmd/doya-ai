...
const tx = {
  ...
  cunningSession: { findUnique: async () => ({
    id: 'session', userId: 'owner', status: 'ended', durationSec: 45, recordingVersion: 2, mode: 'sales',
...
(async () => {
  ...
  assert.equal(recentReads, 1, 'foreign session is rejected before history query');
