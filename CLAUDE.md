...
- ⚠️ Slack Incoming Webhook は**作成時に選んだチャンネルに固定**送信（URLではチャンネルを変えられない）。送信が `ok`（200）なのに届かない時は、そのWebhookの投稿先チャンネル設定を疑う（別チャンネルに配信されている）。

### 認証・初回ログインの副作用（onboarding）

- 新規ユーザーの**初回OAuthログイン時の副作用**（`firstLoginAt` 記録・ドリップ自動エンロール・signup通知）は **`authOptions.events.createUser`** で行う（`src/lib/auth.ts`）。実体は `enrollUserInDripSequences()`（`status:'active'` の `DripSequence` にセグメント条件一致で `DripEnrollment` を作成）。
- ⚠️ NextAuth の `signIn` **コールバック**は、新規ユーザーではDBにユーザーが作成される**前**に走り `user.id` がDB未確定 → `prisma.user.update` が失敗し副作用が取りこぼされる（＝新規登録者にナーチャリングが届かないバグの原因）。`signIn` コールバックは**既存ユーザーのログイン通知・取りこぼし救済のみ**に使うこと。

### 送信メール内リンクの base URL

- ドリップ等の送信メール内リンク・開封/クリックトラッキングURLは **`process.env.NEXTAUTH_URL || 'https://doya-ai.surisuta.jp'`** を使う（コードベース標準）。
- ⚠️ `VERCEL_URL` は**デプロイ保護付きの内部URL**で、クリックするとVercelログイン画面に飛ぶ。メール・外部向けリンクに使ってはいけない。
...
- ⚠️ Vercel環境変数（`vercel env add/rm`）の変更は、**再デプロイするまで本番ランタイムに反映されない**。空コミット push（`git commit --allow-empty`）でデプロイをトリガーする。
- Cron/管理系エンドポイントは `Authorization: Bearer ${CRON_SECRET}` で保護。ローカルから本番を叩いて検証する時は `vercel env pull` で `CRON_SECRET` を取得して付与する。
