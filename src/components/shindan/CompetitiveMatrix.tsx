...
const COLOR_MAP: Record<string, { bg: string; text: string; border: string; barBg: string; scoreBg: string }> = {
  blue:    { bg: 'bg-blue-500/10',    text: 'text-blue-400',    border: 'border-blue-500/20',    barBg: 'bg-blue-500',    scoreBg: 'bg-blue-500/20' },
  violet:  { bg: 'bg-violet-500/10',  text: 'text-violet-400',  border: 'border-violet-500/20',  barBg: 'bg-violet-500',  scoreBg: 'bg-violet-500/20' },
  amber:   { bg: 'bg-amber-500/10',   text: 'text-amber-400',   border: 'border-amber-500/20',   barBg: 'bg-amber-500',   scoreBg: 'bg-amber-500/20' },
  emerald: { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/20', barBg: 'bg-emerald-500', scoreBg: 'bg-emerald-500/20' },
  cyan:    { bg: 'bg-cyan-500/10',    text: 'text-cyan-400',    border: 'border-cyan-500/20',    barBg: 'bg-cyan-500',    scoreBg: 'bg-cyan-500/20' },
  purple:  { bg: 'bg-purple-500/10',  text: 'text-purple-400',  border: 'border-purple-500/20',  barBg: 'bg-purple-500',  scoreBg: 'bg-purple-500/20' },
  red:     { bg: 'bg-red-500/10',     text: 'text-red-400',     border: 'border-red-500/20',     barBg: 'bg-red-500',     scoreBg: 'bg-red-500/20' },
}
  ...
  if (dimKey === 'tracking') {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <p className="text-[10px] font-bold text-teal-400 mb-1.5">自社</p>
          <div className="flex flex-wrap gap-1.5">
            {selfData?.detectedTools?.length > 0
              ...
              : <span className="text-[10px] text-slate-500">未検出</span>
            }
          </div>
          {selfData?.maturityLevel && (
            <p className="text-[10px] text-slate-500 mt-1">成熟度: {selfData.maturityLevel}</p>
          )}
        </div>
        <div>
          <p className="text-[10px] font-bold text-purple-400 mb-1.5">{getHostname(compUrl)}</p>
          <div className="flex flex-wrap gap-1.5">
            {compData?.detectedTools?.length > 0
              ? compData.detectedTools.map((t: string, i: number) => (
                  <span key={i} className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">{t}</span>
                ))
              : <span className="text-[10px] text-slate-500">未検出</span>
            }
          </div>
          {compData?.maturityLevel && (
            <p className="text-[10px] text-slate-500 mt-1">成熟度: {compData.maturityLevel}</p>
          )}
        </div>
      </div>
    )
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          <p className="text-[10px] font-bold text-teal-400 mb-1.5">自社</p>
          {selfData?.heroText && (
            <p className="text-xs text-slate-300 italic mb-1.5">&quot;{selfData.heroText.slice(0, 80)}&quot;</p>
          )}
          <div className="flex flex-wrap gap-1.5">
            {selfData?.heroType && (
        ...
        <div>
          <p className="text-[10px] font-bold text-purple-400 mb-1.5">{getHostname(compUrl)}</p>
          {compData?.heroText && (
            <p className="text-xs text-slate-300 italic mb-1.5">&quot;{compData.heroText.slice(0, 80)}&quot;</p>
          )}
          <div className="flex flex-wrap gap-1.5">
            {compData?.heroType && (
            ...
            {compData?.uspKeywords?.slice(0, 3).map((kw: string, i: number) => (
              <span key={i} className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">{kw}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          {selfData?.userCountText && (
            <p className="text-[10px] text-slate-400 mt-1">ユーザー数表記: {selfData.userCountText}</p>
          )}
        </div>
        <div>
          <p className="text-[10px] font-bold text-purple-400 mb-1.5">{getHostname(compUrl)}</p>
          <div className="flex flex-wrap gap-1.5">
            {compData?.proofElements?.length > 0
              ? compData.proofElements.map((el: string, i: number) => (
                  <span key={i} className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">{el}</span>
                ))
              : <span className="text-[10px] text-slate-500">未検出</span>
            }
          </div>
          {compData?.userCountText && (
            <p className="text-[10px] text-slate-400 mt-1">ユーザー数表記: {compData.userCountText}</p>
          )}
        </div>
      </div>
    )
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {selfData?.ctaPlacement && (
              <span className="text-[10px] bg-slate-700/50 text-slate-400 px-2 py-0.5 rounded-full font-bold">配置: {selfData.ctaPlacement}</span>
            )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 mb-1.5">
            {compData?.ctaTexts?.slice(0, 4).map((cta: string, i: number) => (
              <span key={i} className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">{cta}</span>
            ))}
          </div>
          <div className="flex flex-wrap gap-1.5">
            {compData?.ctaPlacement && (
              <span className="text-[10px] bg-slate-700/50 text-slate-400 px-2 py-0.5 rounded-full font-bold">配置: {compData.ctaPlacement}</span>
            )}
            {compData?.hasLeadMagnet && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">リードマグネット</span>
            )}
            {compData?.hasLiveChat && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">チャット</span>
            )}
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {selfData?.priceIndicators?.map((p: string, i: number) => (
              <span key={i} className="text-[10px] bg-slate-700/50 text-slate-400 px-2 py-0.5 rounded-full font-bold">{p}</span>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {compData?.pricingModel && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">{compData.pricingModel}</span>
            )}
            {compData?.hasFreeTrial && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">無料トライアル</span>
            )}
            {compData?.hasPricingPage && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">料金ページ有</span>
            )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {selfData?.contentTypes?.map((ct: string, i: number) => (
              <span key={i} className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">{ct}</span>
            ))}
            {selfData?.hasNewsletterSignup && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">ニュースレター</span>
            )}
            {selfData?.hasVideo && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">動画</span>
            )}
          </div>
          {selfData?.topicClusters?.length > 0 && (
            <div className="mt-1.5">
              <p className="text-[10px] text-slate-500 mb-0.5">トピック</p>
              <div className="flex flex-wrap gap-1">
                {selfData.topicClusters.slice(0, 4).map((tc: string, i: number) => (
                  <span key={i} className="text-[10px] bg-slate-700/50 text-slate-400 px-1.5 py-0.5 rounded font-bold">{tc}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {compData?.contentTypes?.map((ct: string, i: number) => (
              <span key={i} className="text-[10px] bg-slate-700/50 text-slate-400 px-2 py-0.5 rounded-full font-bold">{ct}</span>
            ))}
            {compData?.hasNewsletterSignup && (
              <span className="text-[10px] bg-slate-700/50 text-slate-400 px-2 py-0.5 rounded-full font-bold">ニュースレター</span>
            )}
            {compData?.hasVideo && (
              <span className="text-[10px] bg-slate-700/50 text-slate-400 px-2 py-0.5 rounded-full font-bold">動画</span>
            )}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {selfData?.positioningType && (
              <span className="text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 px-2 py-0.5 rounded-full font-bold">{selfData.positioningType}</span>
            )}
            {selfData?.hasComparisonPage && (
              <span className="text-[10px] bg-red-500/10 text-red-400 border border-red-500/20 px-2 py-0.5 rounded-full font-bold">比較ページ有</span>
            )}
          </div>
          {selfData?.differentiationClaims?.length > 0 && (
            <div className="mt-1.5">
              <p className="text-[10px] text-slate-500 mb-0.5">差別化ポイント</p>
              <div className="flex flex-wrap gap-1">
                {selfData.differentiationClaims.slice(0, 3).map((dc: string, i: number) => (
                  <span key={i} className="text-[10px] bg-slate-700/50 text-slate-400 px-1.5 py-0.5 rounded font-bold">{dc}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {compData?.positioningType && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">{compData.positioningType}</span>
            )}
            {compData?.hasComparisonPage && (
              <span className="text-[10px] bg-purple-500/10 text-purple-400 border border-purple-500/20 px-2 py-0.5 rounded-full font-bold">比較ページ有</span>
            )}
          </div>
    ...
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6">
      ...
      <div className="mb-6">
        <h3 className="text-lg font-black flex items-center gap-2 mb-1">
          <span className="material-symbols-outlined text-teal-400" style={{ fontSize: 20 }}>
            analytics
          </span>
          <span className="text-white">詳細競合分析マトリクス</span>
          <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-teal-500/20 text-teal-400 border border-teal-500/30">
            7軸分析
          </span>
        </h3>
        <p className="text-xs text-slate-500">自社サイトと競合サイトの多角的な比較分析</p>
      </div>
        ...
        {activeDimensions.map((dim, idx) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] font-bold text-slate-500 text-center max-w-[60px] leading-tight">{dim.label}</span>
            </motion.div>
          )
        })}
      </div>
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className="w-full flex items-center gap-3 bg-slate-800/50 hover:bg-slate-800/80 border border-slate-700/30 rounded-xl px-4 py-3 transition-all group"
              >
                <span className={`material-symbols-outlined ${colors.text}`} style={{ fontSize: 18 }}>
                  {dim.icon}
                </span>
                <span className="text-sm font-bold text-white flex-shrink-0">{dim.label}</span>
                ...
                <motion.span
                  ...
                  className="material-symbols-outlined text-slate-500 group-hover:text-slate-300 transition-colors"
                  style={{ fontSize: 18 }}
                >
                  expand_more
                </motion.span>
              ...
              <AnimatePresence>
                {isExpanded && (
                  ...
                  >
                    <div className="bg-slate-800/30 border border-slate-700/20 rounded-xl p-4 mt-1 space-y-4">
                      {/* 競合ごとの比較 */}
                      {competitors.length > 0 ? (
                            ...
                            return (
                              <div key={ci}>
                                ...
                                <div className="flex items-center gap-2 mb-2">
                                  <span className="text-[10px] font-bold text-slate-500 w-28 truncate">{getHostname(comp.url)}</span>
                                  ...
                                  <span className={`text-xs font-black w-8 text-right ${
                                    compScore != null && selfScore != null
                                      ? compScore > selfScore ? 'text-red-400' : 'text-slate-300'
                                      : 'text-slate-400'
                                  }`}>
                                    {compScore ?? '-'}
                                  </span>
                                ...
                                {ci < competitors.length - 1 && (
                                  <div className="border-t border-slate-700/20 mt-3" />
                                )}
                              </div>
                            )
                          })}
                        ...
                        <p className="text-xs text-slate-500">競合データがありません</p>
                      )}
                      ...
                      {commentary && (
                        <div className={`${colors.bg} border ${colors.border} rounded-xl p-3 mt-3`}>
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <p className="text-[10px] font-bold text-teal-400 mb-0.5">AI分析コメント</p>
      ...
      {detailedComparison?.overallWebPositioning && (
        <div className="mt-6 bg-teal-500/5 border border-teal-500/20 rounded-xl p-4">
          <div className="flex items-start gap-2.5">
            <span className="material-symbols-outlined text-teal-400 flex-shrink-0" style={{ fontSize: 20 }}>
              insights
            </span>
            <div>
              <p className="text-xs font-bold text-teal-400 mb-1">総合Webポジショニング</p>
              <p className="text-sm text-slate-300 leading-relaxed">{detailedComparison.overallWebPositioning}</p>
            </div>
          </div>
        </div>
      )}
