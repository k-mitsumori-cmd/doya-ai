...
function InterviewSidebarImpl({
  ...
  // プラン判定
  const interviewPlanLabel = useMemo(() => {
  ...
  const nextPlanLabel = useMemo(() => {
    if (interviewPlanLabel === 'GUEST' || interviewPlanLabel === 'FREE') return 'PRO'
    if (interviewPlanLabel === 'PRO') return 'ENTERPRISE'
    return 'CONSULT'
  }, [interviewPlanLabel])
  ...
  const isActive = (href: string) => {
    if (href === '/interview') return pathname === '/interview' || pathname === '/interview/create'
    if (href === '/interview/projects') return pathname.startsWith('/interview/projects')
  ...
  const PlanBanner = () => {
    ...
    const planHref = isLoggedIn ? '/interview/dashboard/plan' : '/interview/pricing'
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center shadow-md flex-shrink-0">
              <Zap className="w-4 h-4 text-orange-700 fill-orange-700" />
            </div>
            <p className="text-xs font-black text-white">プラン案内</p>
          </div>
          <p className="text-[11px] text-white font-bold leading-relaxed mb-1">
            現在：{interviewPlanLabel === 'GUEST' ? 'ゲスト' : interviewPlanLabel === 'FREE' ? '無料' : interviewPlanLabel}
          </p>
          <p className="text-[10px] text-orange-100 font-bold leading-relaxed opacity-80">
            {nextPlanLabel === 'PRO' && <>PRO: 月額¥4,980で1日100回まで</>}
            {nextPlanLabel === 'ENTERPRISE' && <>Enterprise: 月額¥49,980で無制限</>}
            {nextPlanLabel === 'CONSULT' && <>さらに上限UP：要相談</>}
          </p>
          <Link
            href={planHref}
            className="mt-2 w-full py-2 bg-white text-orange-700 text-[11px] font-black rounded-lg hover:bg-orange-50 transition-colors shadow-md block text-center"
          >
  ...
  return (
    <>
      <motion.aside
        ...
        className={`${isMobile ? 'relative' : 'fixed left-0 top-0'} h-screen bg-gradient-to-b from-orange-600 via-amber-600 to-orange-700 flex flex-col z-50 shadow-xl`}
      >
        {/* Logo */}
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {INTERVIEW_NAV.map((item) => {
            ...
            const href = item.href === '/interview/pricing' ? (isLoggedIn ? '/interview/dashboard/plan' : '/interview/pricing') : item.href
                ...
                <motion.div
                  ...
                  className={`relative flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer group ${
                    active
                      ...
                      : 'text-orange-100/75 hover:text-white hover:bg-white/10'
                  }`}
                >
                  <Icon
                    className={`w-5 h-5 flex-shrink-0 ${active ? 'text-white' : 'text-orange-200/75 group-hover:text-white'}`}
                  />
                  <AnimatePresence>
                    {showLabel && (
                      <motion.span
                        ...
                        className="text-sm font-semibold whitespace-nowrap overflow-hidden flex-1"
                      >
                        {item.label}
                      </motion.span>
                    )}
                  ...
                  {item.badge && showLabel && (
                    <span className="ml-auto px-1.5 py-0.5 bg-white/20 text-[10px] font-bold text-white rounded-md">
                      {item.badge}
                    </span>
                  )}
                  {active && (
        ...
        {/* User Profile */}
        <div className="p-3 border-t border-white/5 bg-orange-900/25">
          <div className={`flex items-center gap-2 ${!isMobile && isCollapsed ? 'justify-center' : ''}`}>
            <Link
              ...
              title="設定"
            >
              <div className="w-8 h-8 rounded-full bg-orange-600 flex items-center justify-center flex-shrink-0 shadow-inner border border-white/10">
                {session?.user?.image ? (
                  <img
              ...
              <AnimatePresence>
                {showLabel && (
                  ...
                  >
                    ...
                    <p className="text-[10px] font-bold text-orange-100/60 truncate">
                      {interviewPlanLabel === 'GUEST' ? 'ゲスト' : interviewPlanLabel === 'FREE' ? '無料プラン' : `${interviewPlanLabel}プラン`}
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            {showLabel && (
              <div className="flex items-center gap-1 flex-shrink-0">
                {isLoggedIn ? (
                  ...
                  <Link
                    href={`/auth/doyamarke/signin?callbackUrl=${encodeURIComponent(pathname || '/interview')}`}
                    className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-white text-orange-700 text-[10px] font-black hover:bg-orange-50 transition-colors shadow-sm"
                    title="ログイン"
                  >
        ...
        {!isMobile && (
          <button
            onClick={toggle}
            className="absolute -right-3 top-24 w-6 h-6 bg-white rounded-full shadow-lg flex items-center justify-center text-orange-700 hover:bg-orange-50 transition-colors border border-gray-100 z-10"
          >
            {isCollapsed ? (
        ...
        <AnimatePresence>
          {!isCollapsed && (
            ...
            >
              <p className="text-[10px] text-orange-100/35 font-bold tracking-widest">ドヤインタビューAI</p>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.aside>
            ...
            >
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-start gap-2">
                  <div className="w-8 h-8 rounded-full bg-orange-600 text-white flex items-center justify-center text-[11px] font-black flex-shrink-0">
                    AI
                  </div>
...
export const InterviewSidebar = memo(InterviewSidebarImpl)
