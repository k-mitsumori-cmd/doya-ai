...
// ドヤ面接官 アプリ枠（AioAppLayout と同型：デスクトップ/モバイルでサイドバーを配置）
...
import MensetsuSidebar from './MensetsuSidebar'

export default function MensetsuAppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      <div className="hidden md:flex">
        <MensetsuSidebar isCollapsed={sidebarCollapsed} onToggle={(c) => setSidebarCollapsed(c)} />
      </div>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            <MensetsuSidebar forceExpanded isMobile onToggle={() => setMobileMenuOpen(false)} />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        ...
        <div className="sticky top-0 z-30 flex items-center gap-3 border-b border-slate-200 bg-white p-3 md:hidden">
          ...
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined grid h-7 w-7 place-items-center rounded-lg bg-gradient-to-br from-blue-700 to-indigo-800 text-[18px] text-white shadow">
              support_agent
            </span>
            <span className="text-base font-black text-slate-900">ドヤ面接官</span>
          </div>
        </div>
