...
const STRATEGY_NAV: NavItem[] = [
  ...
  { href: '/strategy/settings', label: '設定', icon: Settings },
]
...
function StrategySidebarImpl({
  ...
  // プラン判定
  const strategyPlanLabel = useMemo(() => {
    const strategyPlan = String((session?.user as any)?.strategyPlan || '').toUpperCase()
    const globalPlan = String((session?.user as any)?.plan || '').toUpperCase()
    const p = strategyPlan || globalPlan || (isLoggedIn ? 'FREE' : 'GUEST')
  ...
  const nextPlanLabel = useMemo(() => {
    if (strategyPlanLabel === 'GUEST' || strategyPlanLabel === 'FREE') return 'PRO'
    if (strategyPlanLabel === 'PRO') return 'ENTERPRISE'
    return 'CONSULT'
  }, [strategyPlanLabel])
  ...
  const postLogoutUrl = '/strategy?loggedOut=1'
  ...
  const isActive = (href: string) => {
    if (href === '/strategy') return pathname === '/strategy' || pathname === '/strategy/dashboard'
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center shadow-md flex-shrink-0">
              <Zap className="w-4 h-4 text-indigo-700 fill-indigo-700" />
            </div>
            <p className="text-xs font-black text-white">プラン案内</p>
          </div>
          <p className="text-[11px] text-white font-bold leading-relaxed mb-1">
            現在：{strategyPlanLabel === 'GUEST' ? 'ゲスト' : strategyPlanLabel === 'FREE' ? '無料' : strategyPlanLabel}
          </p>
          ...
          <Link
            href={planHref}
            className="mt-2 w-full py-2 bg-white text-indigo-700 text-[11px] font-black rounded-lg hover:bg-indigo-50 transition-colors shadow-md block text-center"
          >
  ...
  return (
    <>
      <motion.aside
        ...
        className={`${isMobile ? 'relative' : 'fixed left-0 top-0'} h-screen bg-gradient-to-b from-indigo-600 via-purple-600 to-indigo-700 flex flex-col z-50 shadow-xl`}
      >
        {/* Logo */}
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {STRATEGY_NAV.map((item) => {
            const href = item.href === '/strategy/pricing' ? (isLoggedIn ? '/strategy/dashboard/plan' : '/strategy/pricing') : item.href
            ...
            return (
              <Link key={item.href} href={href}>
                ...
                >
                  <Icon
                    className={`w-5 h-5 flex-shrink-0 ${active ? 'text-white' : 'text-indigo-200/75 group-hover:text-white'}`}
                  />
                  <AnimatePresence>
                    {showLabel && (
                      <motion.span
                  ...
                  {active && (
                    <motion.div
                      layoutId="strategyActiveIndicator"
        ...
        {/* User Profile */}
        <div className="p-3 border-t border-white/5 bg-indigo-900/25">
          <div className={`flex items-center gap-2 ${!isMobile && isCollapsed ? 'justify-center' : ''}`}>
            <Link
              href="/strategy/settings"
              className="flex items-center gap-2 p-1.5 rounded-lg hover:bg-white/10 transition-colors cursor-pointer flex-1 min-w-0"
              title="設定"
            >
              <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center flex-shrink-0 shadow-inner border border-white/10">
                {session?.user?.image ? (
                  <img
                ...
                {showLabel && (
                  ...
                  >
                    ...
                    <p className="text-[10px] font-bold text-indigo-100/60 truncate">
                      {strategyPlanLabel === 'GUEST' ? 'ゲスト' : strategyPlanLabel === 'FREE' ? '無料プラン' : `${strategyPlanLabel}プラン`}
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            {showLabel && (
              <div className="flex items-center gap-1 flex-shrink-0">
                {isLoggedIn ? (
                  ...
                  <Link
                    href={`/auth/doyamarke/signin?callbackUrl=${encodeURIComponent(pathname || '/strategy')}`}
                    className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-white text-indigo-700 text-[10px] font-black hover:bg-indigo-50 transition-colors shadow-sm"
                    title="ログイン"
                  >
        ...
        {!isMobile && (
          <button
            onClick={toggle}
            className="absolute -right-3 top-24 w-6 h-6 bg-white rounded-full shadow-lg flex items-center justify-center text-indigo-700 hover:bg-indigo-50 transition-colors border border-gray-100 z-10"
          >
            {isCollapsed ? (
        ...
        <AnimatePresence>
          {!isCollapsed && (
            ...
            >
              <p className="text-[10px] text-indigo-100/35 font-bold tracking-widest">ドヤ戦略AI</p>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.aside>
            ...
            >
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-start gap-2">
                  <div className="w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-[11px] font-black flex-shrink-0">
                    AI
                  </div>
...
// 戦略AI用のツール切替メニュー（ベータ版も含む）
function StrategyToolSwitcher({ showLabel, isCollapsed }: { showLabel: boolean; isCollapsed: boolean }) {
            ...
            >
              <div className="p-2 space-y-1">
                ...
                {TOOLS.map((tool) => {
                  ...
                  const isCurrent = tool.id === 'strategy'
                  return isCurrent ? (
                    <div
                      key={tool.id}
...
export const StrategySidebar = memo(StrategySidebarImpl)
