  ...
  const NavLink = ({ item }: { item: NavItem }) => {
    ...
    return (
      <Link href={item.href}>
        ...
        >
          <Icon className={`w-6 h-6 sm:w-5 sm:h-5 flex-shrink-0 ${active ? 'text-white' : 'text-teal-200/70 group-hover:text-white'}`} />
          ...
          {active && (
            <motion.div
              layoutId="shindanActiveIndicator"
    ...
    <AnimatePresence>
      {!isCollapsed && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="px-3 py-2 text-[10px] font-bold text-teal-200/50 uppercase tracking-wider"
        >
          {title}
        </motion.div>
      )}
  ...
  return (
    <>
      <motion.aside
        ...
        className={`${isMobile ? 'relative' : 'fixed left-0 top-0'} h-screen bg-gradient-to-b from-teal-600 via-teal-700 to-teal-800 flex flex-col z-50 shadow-xl`}
      >
        {/* Logo */}
        ...
        {/* User Profile */}
        <div className="p-2 sm:p-3 border-t border-white/5 bg-teal-900/30">
          <div className={`flex items-center gap-2 ${!isMobile && isCollapsed ? 'justify-center' : ''}`}>
            <div className="flex items-center gap-2 p-1.5 rounded-lg hover:bg-white/10 transition-colors cursor-pointer flex-1 min-w-0">
              <div className="w-9 h-9 sm:w-8 sm:h-8 rounded-full bg-teal-500 flex items-center justify-center flex-shrink-0 shadow-inner border border-white/10">
                {session?.user?.image ? (
                  <img
            ...
            {showLabel && (
              <div className="flex items-center gap-1 flex-shrink-0">
                {isLoggedIn ? (
                  ...
                  <Link
                    href={`/auth/doyamarke/signin?callbackUrl=${encodeURIComponent('/shindan')}`}
                    className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-white text-teal-600 text-[10px] font-black hover:bg-teal-50 transition-colors shadow-sm"
                  >
            ...
            >
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-start gap-2">
                  <div className="w-8 h-8 rounded-full bg-teal-600 text-white flex items-center justify-center text-[11px] font-black flex-shrink-0">
                    AI
                  </div>
