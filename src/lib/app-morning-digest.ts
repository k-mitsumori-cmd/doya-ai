...
// ============================================
// アプリ「朝刊」— 1日1通の統合ダイジェスト（呪い日記 / ゆるせん 共通の組み立て）
//
// 朝に何通も分かれていた通知を1通にまとめる。
//   売上/DL・アプリ内の動き・流入経路・国別・ストア/順位・SNS
// 各セクションは既存レポートの集計関数を deliver:false で呼んで“数字だけ”受け取り、
// ここで短く組み直す。1つ落ちても他は出す（セクション単位で catch）。
//
// 先頭に「きのうを一言で」を置くのが主眼。数字の羅列ではなく、
// 良くなった点を拾って明るく言い切る。悪い時は事実だけ淡々と書く（誇張しない）。
//
// アプリ固有の設定（対象アプリID・売上取得関数・YouTubeチャンネル・通知先など）は
// DigestConfig で受け取る。呪い日記は noroi-morning-digest.ts、
// ゆるせんは yurusen-morning-digest.ts が設定を持つ。
// ============================================

const YT_API = 'https://www.googleapis.com/youtube/v3'

/** 売上レポート（呪い日記/ゆるせんで別関数だが戻り値の形は共通） */
...
/** 自前DBから取るアプリ内指標。用意できないアプリでは省略できる */
...
export type DigestConfig = {
  /** 見出しに出すアプリ名 */
  appLabel: string
  /** App Store の数値ID（流入経路・国別・順位の対象） */
  appId: string
  /** 通知先。先に見つかった環境変数を使う */
  webhookEnvs: string[]
  /** 売上/DL の取得 */
  fetchSales: () => Promise<SalesLike>
  /** アプリ内の動き（未対応アプリは省略） */
  fetchEngagement?: () => Promise<EngagementLike>
  /** アプリ内指標が未接続の場合に朝刊へ出す注記 */
  engagementNote?: string
  /** キーワード順位の追跡語（未指定は既定セット） */
  marketingKeywords?: string[]
  /** 順位スナップショットの SystemSetting キー（アプリごとに分ける） */
  marketingSnapshotKey: string
  /** SNS欄に出す YouTube チャンネル */
...
async function postSlack(cfg: DigestConfig, text: string): Promise<void> {
  const url = cfg.webhookEnvs.map((k) => process.env[k]).find((v) => !!v)
  if (!url) throw new Error(`Slack webhook URL is not set（${cfg.webhookEnvs.join(' / ')}）`)
...
async function fetchYoutube(cfg: DigestConfig): Promise<YtRow[]> {
  const apiKey = process.env.YOUTUBE_API_KEY
  if (!apiKey) throw new Error('YOUTUBE_API_KEY is not set')
  if (cfg.youtubeChannels.length === 0) return []

  let snap: YtSnap = {}
  try {
    const row = await prisma.systemSetting.findUnique({ where: { key: cfg.youtubeSnapshotKey } })
  ...
  const qs = new URLSearchParams({
    ...
    id: cfg.youtubeChannels.map((c) => c.id).join(','),
    key: apiKey,
  })
  ...
  for (const ch of cfg.youtubeChannels) {
  ...
  try {
    await prisma.systemSetting.upsert({
      where: { key: cfg.youtubeSnapshotKey },
      create: { key: cfg.youtubeSnapshotKey, value: JSON.stringify(next) },
...
/**
 * 設定に従って朝刊を組み立て、Slack に1通で送る。
 * @param opts.deliver false なら送信せず本文だけ返す（動作確認用）
 */
export async function sendAppMorningDigest(
  cfg: DigestConfig,
  ...
  const sales = await cfg.fetchSales().catch((e) => {
    errors.push(`売上/DL: ${e instanceof Error ? e.message : String(e)}`)
    return null
  })
  const engagement = cfg.fetchEngagement
    ? await cfg.fetchEngagement().catch((e) => {
        errors.push(`アプリ内の動き: ${e instanceof Error ? e.message : String(e)}`)
        return null
      })
    : null
  const source = await sendAppStoreSourceReport({
    deliver: false,
    appId: cfg.appId,
  ...
  const country = await sendAppStoreCountryReport({
    deliver: false,
    appId: cfg.appId,
    appLabel: cfg.appLabel,
  }).catch((e) => {
  ...
  const marketing = await sendAppStoreMarketingReport({
    deliver: false,
    appId: cfg.appId,
    appLabel: cfg.appLabel,
    keywords: cfg.marketingKeywords,
    snapshotKey: cfg.marketingSnapshotKey,
  }).catch((e) => {
    errors.push(`ストア/順位: ${e instanceof Error ? e.message : String(e)}`)
    return null
  })
  const youtube = await fetchYoutube(cfg).catch((e) => {
  ...
  if (!engagement && cfg.engagementNote) {
    L.push('')
    L.push('■ アプリ内の動き')
    L.push(`　${cfg.engagementNote}`)
  }
  ...
  let message = L.join('\n')
  const opsKey = cfg.appLabel === '呪い日記' ? 'noroi' : cfg.appLabel === 'ゆるせん' ? 'yurusen' : null;
  if (opsKey) message += await dailyOperationsSection(opsKey);
  if (deliver) {
    await postSlack(cfg, message);
    if (opsKey) await recordReportDelivered(opsKey + '-daily');
  }
