 ...
 */
import { stripe, resolvePlanIdFromSubscription, planTierFromPlanId } from '@/lib/stripe'
...
export async function runBillingAudit(windowHours = 24): Promise<BillingAudit> {
  ...
  const live = all.filter((s) => ACTIVE_LIKE.has(String(s.status)))
...
export function formatBillingAuditMessage(audit: BillingAudit, opts: { windowLabel: string }): string {
  const lines: string[] = []
  const hasProblem = !audit.webhookOk || audit.mismatched.length > 0 || audit.duplicates.length > 0
  ...
  if (!hasProblem) {
    lines.push('')
    lines.push('整合性チェック: 異常なし（Webhook正常・反映漏れ0・重複0）')
  }
