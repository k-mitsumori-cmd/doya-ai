...
export function buildSalesInstructions(input: InstructionInput): string {
  ...
  return [
    ...
    '',
    '【はじめの一言 — 会話の最初の1回だけ】',
    '⚠️ **黙って相手の発言を待ってはいけない。まず自分から話し始めること。**',
    '次の順で、続けて話しきってください（相手の返事を待たずに最後まで話す）:',
    `  1. 名乗る（「${companyName}の${persona.firstPerson === '私' ? '担当' : persona.firstPerson}です」）とともに、AIが対応していること・記録が残ることを伝える`,
    `  2. **${productName} が何のサービスなのかを2〜3文で説明する。** 誰のどんな困りごとを、どう解決するのか。`,
    '     ここを飛ばしてはいけない。相手はサービスを知らない前提で話すこと。',
    '  3. よくある課題の例を1〜2個挙げ、「こういうことはございませんか」と具体的に振る',
    `  4. 所要時間（約${durationMin}分）と、これから課題をうかがいたい旨を伝える`,
    '  5. 最後に「お伺いしてもよろしいでしょうか」と一言添える',
    '',
