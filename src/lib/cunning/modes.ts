...
export const MODES: Record<CunningMode, ModeDef> = {
  ...
  oneonone: {
    ...
    inputLabel: '相手の発言',
    outputHint: {
      summary: '次に返す一言（共感や問い・20〜30文字）',
      script: 'そのまま言える自然な会話の返し（共感→深掘りの問い/前向きFB・1〜3文）',
    },
    persona: [
      'あなたは1on1面談を進める人をリアルタイムで支援するアシスタントです。',
      '相手（部下/メンバー）の発言に対し、質問でなくても【会話として自然に次に返すセリフ】を必ず1つ作ります。',
      'まず共感・受け止めをし、詰めずに本音や背景を引き出す問い返し、または前向きなフィードバックにします。',
      '心理的安全性を大切にし、傾聴の姿勢で。説教・否定・誘導尋問は避けてください。',
    ],
    prepInstruction:
      '1on1で相手が言いそうな発言（悩み・相談・雑談・不満など）を想定し、共感＋良い問い返し/FBの会話例を作ってください。',
  },
  grilling: {
    id: 'grilling',
    label: '激詰め対応',
    ...
    inputLabel: '上司/相手の追及',
    outputHint: {
      summary: 'まず返す一言（受け止め/結論・20〜30文字）',
      script: '会話として言える落ち着いた切り返し（受け止め→結論→事実→打ち手、必要なら丁寧に時間を買う・1〜3文）',
    },
    persona: [
      'あなたは厳しい追及・詰めを受けている人をリアルタイムで支援するアシスタントです。',
      '相手の発言（多くは質問でなく叱責・追及）に対し、【会話として自然に冷静に返すセリフ】を必ず1つ作ります。',
      'まず受け止め、結論ファースト→事実→リカバリ策の順で。対立・言い訳・感情的反応は避け、誠実で前向きに。',
      '即答できない時は、丁寧に確認の時間をもらう言い回しを提案します。卑屈になりすぎないこと。',
    ],
    prepInstruction:
