...
export function buildImagePrompt(input: BuildPromptInput): string {
  ...
  const lines: string[] = [
    ...
    // ⚠️ 作風が指定されているときは、ここを「厳守」にしてはいけない。
    //    極太・面積40%・帯を敷く・塗りCTA を強制すると、どの作風を選んでも
    //    同じ量販チラシの絵になる（2026-09-01に実機で確認。落ち着いた雑誌風を
    //    選んだのに金地の情報商材風が出た）。可読性の下限だけ残し、
    //    書体・帯・CTAの見せ方は作風に委ねる。
    designRefPrompt
      ? [
          '■ 文字の描き方（上の作風に従うこと）:',
          `  大見出しは1行あたり画像の高さの${Math.max(6, headlinePct - 3)}%以上を目安に、遠目で読める大きさにする。`,
          '  背景と十分なコントラストを取り、文字が背景に埋もれないようにする。',
          '  ⚠️ 書体・帯やプレートの有無・CTAの見せ方は、**上の作風の指定に従う**こと。',
          '     太さや装飾を作風より優先しない。作風が細身・余白重視ならそれに従う。',
        ].join('\n')
      : [
          '■ 文字の描き方（最重要・厳守）:',
          ...
          isVertical ? '  縦長なので、文字は横幅いっぱいを使って大きく組む。左右に余らせないこと。' : '',
        ]
          .filter(Boolean)
          .join('\n'),
    '',
