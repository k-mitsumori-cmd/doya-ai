...
import { safeFetchText, htmlToText, type SafeFetchFailure } from '@/lib/net/safe-fetch'
import { geminiGenerateJson, GEMINI_TEXT_MODEL_DEFAULT } from '@seo/lib/gemini'
import type { BrandProfile } from './types'

const MAX_CHARS = 14000

export class BrandSourceError extends Error {
  constructor(public readonly failure: SafeFetchFailure | { reason: 'insufficient_text' }) {
    super('このサイトを自動で読み取れませんでした。URLを確認するか、サービスの説明文を入力して続けてください。')
    this.name = 'BrandSourceError'
  }

  get status(): number {
    return ['network', 'timeout', 'body'].includes(this.failure.reason)
      || (this.failure.reason === 'http' && (this.failure.status ?? 0) >= 500) ? 503 : 422
  }
}
...
export async function analyzeBrand(sourceUrl: string, manualText?: string): Promise<BrandProfile> {
  const base = new URL(sourceUrl)
  let html: string | null = null
  let text = manualText?.trim().slice(0, MAX_CHARS) || ''
  if (!text) {
    let failure: SafeFetchFailure = { reason: 'network' }
    html = await safeFetchText(base.toString(), {
      timeoutMs: 15000, maxRedirects: 5,
      onFailure: (detail) => { failure = detail },
    })
    if (!html) throw new BrandSourceError(failure)
    text = htmlToText(html).slice(0, MAX_CHARS)
    if (text.length < 150) throw new BrandSourceError({ reason: 'insufficient_text' })
  }

  const colors = html ? await extractColors(html, base) : []
  const logoUrl = html ? extractLogo(html, base) : undefined

  const prompt = [
    'あなたは広告クリエイティブのディレクターです。',
    '以下の提供されたサービス説明から、広告制作に使うブランド情報を抽出してください。',
    '',
    '【制約】',
    '- 提供された内容だけから抽出してください。あなたの知識で補完しないでください。',
    ...
    '}',
    '',
    manualText ? '【利用者が入力したサービス説明】' : '【サイトの内容】',
    text,
  ].join('\n')
