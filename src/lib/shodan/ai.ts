...
export async function analyzeCompany(research: CompanyResearch, own?: OwnCompanyProfile | null): Promise<CompanyAnalysis> {
  ...
  const prompt = [
    ...
    '- すべての仮説・指摘は、必ず調査事実（従業員数/マーケ実施状況/保有サイト・チャネル/プレスリリース 等）に紐づける。事実が乏しい箇所は「未確認」と明示し、断定しすぎない。',
...
export async function generateSlides(
  ...
  const prompt = [
    ...
    '3. type=content: 御社の現状理解（調査事実の要点。数値は調査事実に明記がある場合のみ使い、無ければ書かない）',
    ...
    '',
    '# 制約: 事実にない数値・固有名詞を創作しない。計測していない指標（記事数・更新頻度など）は断定しない。調査事実と矛盾しない。敬体。',
    '',
    '# 調査事実', facts,
