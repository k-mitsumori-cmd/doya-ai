...
function BannerTestPageInner() {
  ...
  // プランティアフィルタ
  const [activePlanFilter, setActivePlanFilter] = useState<PlanFilterType>('すべて')
  ...
  const categoryMapping: { [key: string]: string } = {
    ...
    '金融・保険': '医療・金融',
    'EC・セール': 'EC',
    // 2026-08-23 投入分（テンプレート画像刷新v2）の industry 値。
    // ⚠️ ここに無いと categoryOrder に載らず「その他」扱いで最下部に落ちる。
    '飲料・食品': '食品・飲料',
    '健康・フィットネス': 'スポーツ・趣味',
    'IT・SaaS': 'ビジネス',
    '教育・セミナー': '教育',
  ...
  // グリッドギャラリー用：全カテゴリグループ（プランフィルタ反映）
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent, category: string) => {
    ...
    touchOriginalStartX.current[category] = e.touches[0].clientX // 慣性スクロール用に元の開始位置を保持
  ...
  const handleTouchEnd = useCallback((e: React.TouchEvent, category: string) => {
    ...
    const container = scrollRefs.current[category]
    if (container) {
      // スワイプの勢いを計算してスムーズにスクロール（元の開始位置との差分を使用）
      const diff = touchOriginalStartX.current[category] - touchCurrentX.current[category]
      if (Math.abs(diff) > 30) {
      ...
      <main className="flex-1 ml-0 md:ml-[240px] min-h-screen bg-black overflow-x-hidden">
        ...
        <div className="relative pt-12 md:pt-0">
          ...
          <div
            ...
            className={`fixed top-12 md:top-0 left-0 md:left-[240px] right-0 z-20 overflow-hidden transition-all duration-500 ease-in-out ${
              isHeroCollapsed
          ...
          <div className={`w-full relative z-10 bg-black/90 backdrop-blur-sm border-b border-gray-800/50 transition-all duration-500 ease-in-out ${
            ...
            {/* プランフィルタタブ */}
            <div className="flex items-center gap-1 px-2 sm:px-4 md:px-8 lg:px-12 py-1.5 border-b border-gray-800/30 overflow-x-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              <span className="flex-shrink-0 text-[10px] sm:text-xs text-gray-500 mr-1">プラン:</span>
              {([
                ...
                <button
                  key={tab.key}
                  onClick={() => setActivePlanFilter(tab.key)}
                  className={`flex-shrink-0 px-2.5 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs font-medium transition-all whitespace-nowrap ${
                    activePlanFilter === tab.key ? tab.color + ' shadow-lg' : tab.inactive
                  }`}
                >
                  {tab.label}
                </button>
                            ...
                            {isLocked && (
                              <>
                                ...
                                <div className={`absolute top-1 right-1 flex items-center gap-0.5 px-1 py-0.5 rounded text-[7px] font-bold text-white z-20 ${
                                  lockType === 'enterprise' ? 'bg-purple-500/90' : lockType === 'login' ? 'bg-red-500/90' : 'bg-amber-500/90'
                                }`}>
                                  <Lock className="w-2 h-2" />
                                  {lockType === 'enterprise' ? 'Enterprise' : lockType === 'login' ? 'ログイン' : 'PRO'}
                                </div>
                              </>
                            )}
                          </motion.div>
