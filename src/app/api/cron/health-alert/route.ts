import { observeServiceHealth } from '@/lib/service-operations-monitor';
...
async function checkTarget(t: Target): Promise<{ name: string; ok: boolean; rtt: number; status?: number }> {
  ...
  try {
    ...
    await observeServiceHealth(t.key, t.name, res.ok).catch(() => {});
    ...
    await observeServiceHealth(t.key, t.name, false).catch(() => {});
