...
// GET  /api/quote/invite/[token] — 招待の内容（組織名・権限）
// POST /api/quote/invite/[token] — 招待を受ける（要ログイン）
import { NextRequest, NextResponse } from 'next/server'
import { prisma } from '@/lib/prisma'
import { resolveUserId } from '@/lib/quote/access'
...
async function load(token: string) {
  ...
  return prisma.quoteMember.findUnique({
...
export async function POST(_req: NextRequest, ctx: Ctx) {
  ...
  const already = await prisma.quoteMember.findFirst({
  ...
  await prisma.quoteMember.update({
    where: { id: m.id },
    data: {
      userId,
      status: 'ACTIVE',
