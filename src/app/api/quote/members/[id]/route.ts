...
// PATCH  /api/quote/members/[id] — 権限を変更
// DELETE /api/quote/members/[id] — メンバーを外す／招待を取り消す
...
async function load(id: string, organizationId: string) {
  ...
  return prisma.quoteMember.findFirst({
...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  const role = body?.role as QuoteRole
  ...
  if (m.role === 'owner' || ROLE_HIERARCHY[m.role as QuoteRole] >= ROLE_HIERARCHY[c.role]) {
  ...
  await prisma.quoteMember.update({ where: { id: m.id }, data: { role } })
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  if (ROLE_HIERARCHY[m.role as QuoteRole] >= ROLE_HIERARCHY[c.role]) {
    return NextResponse.json({ error: 'このメンバーは外せません' }, { status: 403 })
  }

  await prisma.quoteMember.delete({ where: { id: m.id } })
