...
function createFallbackPersona(args: {
  ...
  const nameKana = kanaMap[name] || 'さとう けんた'
    ...
    if (!disableLimits && isGuest) {
      ...
      if (dailyLimit !== -1 && dailyUsed + 1 > dailyLimit) {
        await notifyApiError(
          new Error('本日の生成上限に達しました（ゲスト）'),
          req,
          429,
          { dailyLimit, dailyUsed, userId: 'guest' }
        )
        return NextResponse.json(
          {
      ...
      try {
        ...
        if (!isFreeHourActive && dailyLimit !== -1 && normalized.dailyUsage + 1 > dailyLimit) {
          await notifyApiError(
            new Error('本日の生成上限に達しました'),
            request,
            429,
            { dailyLimit, dailyUsed: normalized.dailyUsage, userId }
          )
          return NextResponse.json(
            {
...
`
    ...
    await notifyApiError(error instanceof Error ? error : new Error(String(error)), request, 500)
    return NextResponse.json(
