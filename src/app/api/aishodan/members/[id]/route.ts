...
async function load(id: string, organizationId: string) {
  ...
  return prisma.aishodanMember.findFirst({
...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  if (m.role === 'owner' || ROLE_HIERARCHY[m.role as AishodanRole] >= ROLE_HIERARCHY[c.role]) {
  ...
  await prisma.aishodanMember.update({ where: { id: m.id }, data: { role } })
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  if (ROLE_HIERARCHY[m.role as AishodanRole] >= ROLE_HIERARCHY[c.role]) {
