export const runtime = 'nodejs'
export const dynamic = 'force-dynamic'
export const maxDuration = 300

import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '@/lib/auth'
import { geminiGenerateText, GEMINI_TEXT_MODEL_DEFAULT } from '@seo/lib/gemini'

type ToolType = 'form' | 'email' | 'phone'

interface Input {
  type: ToolType
  companyName?: string
  industry?: string
  contactPerson?: string
  myService?: string         // 自社サービス概要
  myCompany?: string         // 自社名
  benefit?: string           // 訴求ポイント
  tone?: 'formal' | 'casual' | 'friendly'
}

function buildPrompt(input: Input): string {
  const {
    type,
    companyName = '貴社',
    industry = '',
    contactPerson = 'ご担当者様',
    myService = '弊社サービス',
    myCompany = '弊社',
    benefit = '',
    tone = 'formal',
  } = input

  const toneLabel = tone === 'casual' ? 'カジュアル' : tone === 'friendly' ? '親しみやすい' : '丁寧でフォーマル'

  if (type === 'form') {
    return [
      `以下の条件で、企業のお問い合わせフォーム送信用の営業文を作成してください。`,
      ``,
      `■ 相手企業: ${companyName}${industry ? `（${industry}業界）` : ''}`,
      `■ 担当者: ${contactPerson}`,
      `■ 自社名: ${myCompany}`,
      `■ 自社サービス: ${myService}`,
      `■ 訴求ポイント: ${benefit || '業務効率化と成果向上'}`,
      `■ トーン: ${toneLabel}`,
      ``,
      `要件:`,
      `- 400〜600字程度`,
      `- 件名は不要、本文のみ`,
      `- 一方的な売り込みは避け、相手企業の状況を尊重する`,
      `- 最後に簡単な「次のアクション」を提案する（例: 15分の打ち合わせ）`,
      `- 装飾記号や絵文字は使わない`,
      ``,
      `出力は本文テキストのみ。説明や前置きは不要です。`,
    ].join('\n')
  }

  if (type === 'email') {
    return [
      `以下の条件で、新規開拓メールを作成してください。`,
      ``,
      `■ 相手企業: ${companyName}${industry ? `（${industry}業界）` : ''}`,
      `■ 担当者: ${contactPerson}`,
      `■ 自社名: ${myCompany}`,
      `■ 自社サービス: ${myService}`,
      `■ 訴求ポイント: ${benefit || '業務効率化と成果向上'}`,
      `■ トーン: ${toneLabel}`,
      ``,
      `要件:`,
      `- 件名（30字以内）と本文（300〜500字）を作成`,
      `- 件名は開封率の高い表現にする`,
      `- 本文は導入→価値提案→次のアクションの3パート`,
      `- 装飾記号や絵文字は使わない`,
      ``,
      `出力形式:`,
      `件名: 〜〜`,
      ``,
      `本文:`,
      `（本文）`,
    ].join('\n')
  }

  // phone
  return [
    `以下の条件で、新規開拓の電話スクリプト（荷電スクリプト）を作成してください。`,
    ``,
    `■ 相手企業: ${companyName}${industry ? `（${industry}業界）` : ''}`,
    `■ 担当者: ${contactPerson}`,
    `■ 自社名: ${myCompany}`,
    `■ 自社サービス: ${myService}`,
    `■ 訴求ポイント: ${benefit || '業務効率化と成果向上'}`,
    `■ トーン: ${toneLabel}`,
    ``,
    `要件:`,
    `- 受付突破→担当者接続→自己紹介→価値提案→アポ獲得の流れ`,
    `- セリフはすべて「自分:」「相手:」形式で書く`,
    `- 「お忙しいところ」「少しだけお時間」など断られにくいフレーズを使う`,
    `- 3〜5回のやりとりを想定`,
    `- 想定される断り文句と切り返しトークも追記`,
    ``,
    `出力形式:`,
    `【トークスクリプト】`,
    `自分: 〜〜`,
    `相手: 〜〜`,
    `自分: 〜〜`,
    `...`,
    ``,
    `【切り返しトーク】`,
    `- 断り例1: 〜 → 切り返し: 〜`,
    `- 断り例2: 〜 → 切り返し: 〜`,
  ].join('\n')
}

export async function POST(req: NextRequest) {
  try {
    const session = await getServerSession(authOptions)
    const userId = (session?.user as any)?.id
    if (!userId) {
      return NextResponse.json({ error: 'ログインが必要です' }, { status: 401 })
    }

    const body = (await req.json().catch(() => null)) as Input | null
    if (!body || !['form', 'email', 'phone'].includes(body.type)) {
      return NextResponse.json({ error: 'typeは form/email/phone のいずれかを指定してください' }, { status: 400 })
    }

    const prompt = buildPrompt(body)

    const text = await geminiGenerateText({
      model: GEMINI_TEXT_MODEL_DEFAULT,
      parts: [{ text: prompt }],
    })

    if (!text || typeof text !== 'string') {
      return NextResponse.json({ error: 'AI生成に失敗しました' }, { status: 502 })
    }

    return NextResponse.json({ success: true, text: text.trim() })
  } catch (e: any) {
    console.error('[doyalist/tools]', e)
    return NextResponse.json({ error: e?.message || 'ツール実行に失敗しました' }, { status: 500 })
  }
}
