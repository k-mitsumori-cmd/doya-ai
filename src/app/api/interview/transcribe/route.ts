...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userId) {
      ...
      console.log('[INTERVIEW] Transcribe API - Plan resolution logic:', {
        unifiedPlan,
        unifiedPlanUpper,
      ...
      if (unifiedPlanUpper === 'ENTERPRISE') {
        ...
        if (subscriptionPlanUpper === 'ENTERPRISE') {
          planToUse = 'ENTERPRISE'
          console.log('[INTERVIEW] Transcribe API - Using ENTERPRISE (subscription plan is ENTERPRISE)')
        } else {
          planToUse = 'PRO'
          console.log('[INTERVIEW] Transcribe API - Using PRO (unified plan is PRO)')
        }
      } else {
        // それ以外の場合、UserServiceSubscriptionのプランがあればそれを使用、なければ統一プランを使用
        planToUse = subscription?.plan || unifiedPlan
        console.log('[INTERVIEW] Transcribe API - Using subscription plan or unified plan:', planToUse)
      }
      
      plan = await getUserPlan(userId, null, planToUse)
      
      console.log('[INTERVIEW] Transcribe API - Final resolved plan:', {
        userId,
        subscriptionPlan: subscription?.plan,
        unifiedPlan,
        planToUse,
    ...
    if (error?.code === 3) {
      ...
      errorDetails = '音声ファイルのエンコーディングが正しくないか、サポートされていない形式です。'
      statusCode = 400
    } else if (error?.code === 7) {
      errorMessage = '認証エラー'
      ...
      errorMessage = '内部エラー'
      errorDetails = 'Google Cloud Speech-to-Text APIで内部エラーが発生しました。'
      statusCode = 500
    } else if (error?.response?.status === 400) {
      errorMessage = 'リクエストが無効です'
      errorDetails = error?.response?.data?.error || error?.message || 'リクエストパラメータが正しくありません。'
      statusCode = 400
      ...
      errorDetails = 'Google Cloud Storage上のファイルが見つかりません。'
      statusCode = 404
    } else if (error?.response?.status === 429) {
      errorMessage = 'リクエストが多すぎます'
      errorDetails = 'APIのレート制限に達しました。しばらく待ってから再度お試しください。'
      statusCode = 429
    } else if (error?.response?.status === 500 || error?.response?.status === 502 || error?.response?.status === 503) {
      errorMessage = 'サーバーエラー'
      errorDetails = 'Google Cloud Speech-to-Text APIでサーバーエラーが発生しました。しばらく待ってから再度お試しください。'
      statusCode = 500
    } else if (error?.code === 'ENOTFOUND' || error?.code === 'ECONNREFUSED') {
      errorMessage = 'ネットワークエラー'
      errorDetails = 'Google Cloud Speech-to-Text APIに接続できませんでした。ネットワーク接続を確認してください。'
      statusCode = 503
    } else if (error?.name === 'AbortError' || error?.message?.includes('timeout')) {
      errorMessage = 'タイムアウトエラー'
      errorDetails = 'リクエストがタイムアウトしました。ファイルが大きすぎる可能性があります。'
      statusCode = 504
    }
