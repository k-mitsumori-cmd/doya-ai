...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  const target = await prisma.aioMember.findFirst({ where: { id: p.id, organizationId: sctx.organizationId } })
  ...
  const updated = await prisma.aioMember.update({ where: { id: target.id }, data: { role } })
...
export async function DELETE(req: NextRequest, ctx: Ctx) {
  ...
  await prisma.aioMember.delete({ where: { id: target.id } })
