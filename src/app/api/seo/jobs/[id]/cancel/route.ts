...
export async function POST(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const job = await (prisma as any).seoJob.findFirst({ where: { id, article: owner }, select: { id: true, status: true, supersededAt: true, articleId: true } })
    ...
    const updated = await prisma.$transaction(async (tx) => {
      await tx.seoArticle.update({ where: { id: job.articleId, ...owner }, data: { status: 'ERROR' } })
      return tx.seoJob.update({
        where: { id, article: owner, status: job.status, supersededAt: null },
        data: { executionToken: null, executionExpiresAt: null, status: 'cancelled', error: 'cancelled by user', finishedAt: new Date() },
      })
    })
