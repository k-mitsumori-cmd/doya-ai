...
function defaultDiagramTemplate() {
  return [
    ...
    '"ビジネスライクで洗練された、わかりやすい図解画像"を制作することです。',
    '',
    '【前提】',
    ...
    '',
    '【デザインの方向性】',
    '・スタイル：ビジネスライク、洗練された、プロフェッショナル',
    '・カラー：落ち着いたビジネスカラー（青・グレー・控えめなアクセントカラー）。ポップで明るい色は避ける',
    '・雰囲気：ビジネスプレゼンテーションや企業レポートに適した印象',
    '・レイアウト：整理された、明確な視覚的階層',
    '・要素：幾何学的な図形、プロフェッショナルなアイコン、矢印、コネクター',
    '・避けるべき要素：遊び心のある、漫画風、過度に装飾的な要素',
    '・余白：明確さとプロフェッショナルさのため、十分な余白を確保',
    '・コントラスト：読みやすさのため高コントラスト、ただしプロフェッショナルなトーンを維持',
    '・背景：白または薄い単色で、視認性を最優先',
    '',
    '【元となる記事内容】',
    '{{ARTICLE_CONTENT}}',
  ].join('\n')
...
{"diagrams": [{"title": "図解タイトル", "description": "図解の内容説明（Geminiが画像生成できるよう詳細に）"}]}
      ...
      if (!diagrams.length) {
        ...
        diagrams = base.map((t: string, i: number) => ({
          title: t || `図解 ${i + 1}`,
          description: `記事「${title}」の「${t}」の内容を、ビジネスライクで洗練された図解として表現してください。` +
            ` 箇条書き/フロー/比較（必要に応じて）で、プロフェッショナルなアイコン・矢印・枠線を使い、` +
            ` 落ち着いたビジネスカラー（青・グレー系）で、白背景に余白を多めに取った構成にしてください。`,
        }))
      }
