...
function defaultDiagramTemplate() {
  return [
    'あなたは「ビジネス記事用の図解制作」を専門とするトップクラスのデザイナーです。',
    '今回の目的は、オウンドメディアの記事内容を一瞬で理解できる',
    '"ビジネスライクで洗練された、わかりやすくシンプルな図解画像"を制作することです。',
    '',
    '【最重要：デザインスタイル】',
    '・絶対にポップで明るい色、派手な色、カラフルな配色は使わない',
    '・落ち着いたビジネスカラー（青・グレー・控えめなアクセント）のみを使用',
    '・企業の公式資料やビジネス書に掲載されるような、プロフェッショナルで信頼感のあるデザイン',
    '・遊び心のある、漫画風、ポップな要素は一切使わない',
    '',
    '【前提】',
    '・使用用途：オウンドメディアのSEO記事内・サムネイル',
    '・ターゲット：ビジネスパーソン、専門知識がない人でも直感的に理解できる読者',
    '・記事内容：下記の本文内容を正確に要約・視覚化すること',
    '・難しい表現や抽象表現は使わない',
    '・「見ただけで内容が分かる」ことを最優先する',
    '',
    '【デザインの方向性】',
    '・スタイル：ビジネスライク、洗練された、プロフェッショナル、シンプルで明確',
    '・カラー：落ち着いたビジネスカラー（青・グレー・控えめなアクセントカラー）のみを使用。ポップで明るい色、派手な色、カラフルな配色は一切使わない',
    '・雰囲気：ビジネスプレゼンテーションや企業レポート、専門書に適した印象',
    '・レイアウト：整理された、明確な視覚的階層、シンプルで洗練された構成',
    '・要素：幾何学的な図形、プロフェッショナルなアイコン、矢印、コネクター',
    '・避けるべき要素：遊び心のある、漫画風、過度に装飾的な要素、ポップなデザイン、カラフルな配色、明るすぎる色',
    '・余白：明確さとプロフェッショナルさのため、十分な余白を確保',
    '・コントラスト：読みやすさのため高コントラスト、ただしプロフェッショナルなトーンを維持',
    '・背景：白または薄い単色（グレー系）で、視認性を最優先',
    '・全体的な印象：企業の公式資料やビジネス書に掲載されるような、信頼感のあるデザイン',
    '',
    '【元となる記事内容】',
    '{{ARTICLE_CONTENT}}',
  ].join('\n')
...
【重要：デザイン要件】
- ビジネスライクで洗練された、プロフェッショナルな図解
- 落ち着いたビジネスカラー（青・グレー系）のみを使用
- ポップで明るい色、派手な色、カラフルな配色は一切使わない
- 企業の公式資料やビジネス書に掲載されるような信頼感のあるデザイン
- シンプルで明確、整理された構成
...
出力形式（JSONのみ）:
{"diagrams": [{"title": "図解タイトル", "description": "図解の内容説明（ビジネスライクで洗練されたデザインで、落ち着いたビジネスカラーのみを使用。ポップで明るい色は一切使わない。Geminiが画像生成できるよう詳細に）"}]}
      `
      ...
      if (!diagrams.length) {
        ...
        diagrams = base.map((t: string, i: number) => ({
          title: t || `図解 ${i + 1}`,
          description: `記事「${title}」の「${t}」の内容を、ビジネスライクで洗練された、シンプルで明確な図解として表現してください。` +
            ` 箇条書き/フロー/比較（必要に応じて）で、プロフェッショナルなアイコン・矢印・枠線を使い、` +
            ` 落ち着いたビジネスカラー（青・グレー系）のみを使用し、ポップで明るい色は一切使わない。` +
            ` 白または薄いグレー背景に余白を多めに取った、企業の公式資料やビジネス書に掲載されるような信頼感のある構成にしてください。`,
        }))
      }
      ...
      if (diagrams.length) {
        ...
        for (const diagram of diagrams) {
          const prompt = [
            ...
            '',
            '【厳守事項】',
            '- カラー：落ち着いたビジネスカラー（青・グレー・控えめなアクセント）のみ。ポップで明るい色、派手な色、カラフルな配色は絶対に使わない',
            '- スタイル：企業の公式資料やビジネス書に掲載されるような、プロフェッショナルで信頼感のあるデザイン',
            '- 避けるべき：遊び心のある、漫画風、ポップな、カラフルな、明るすぎる要素',
            '- 背景：白または薄いグレー系のみ',
            '- 全体的な印象：ビジネスプレゼンテーションや企業レポートに適した、洗練されたシンプルなデザイン',
          ]
            .filter(Boolean)
            .join('\n')
