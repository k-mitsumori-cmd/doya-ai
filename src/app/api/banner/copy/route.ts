...
function buildCopyPrompt(input: CopyRequest) {
  ...
  const system = [
    ...
    '',
    '╔═══════════════════════════════════════════════════════════════╗',
    '║ 🎯 最重要：業種「' + categoryConfig.name + '」に特化したコピーを生成すること ║',
    '╚═══════════════════════════════════════════════════════════════╝',
    '',
    '【業種】' + categoryConfig.name,
    '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
    '■ この業種で効果の高いキーワード（必ず各コピーに1つ以上含める）:',
    '  → ' + categoryConfig.keywords.join(' / '),
    '',
    '■ ターゲット顧客像:',
    '  → ' + categoryConfig.targetHint,
    '',
    '■ この業種の高CTRコピー例（参考にすること）:',
    ...
    '【絶対ルール】',
    '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
    '1. ⚠️ 必ず「' + categoryConfig.name + '」業種に関連するコピーを生成すること',
    '2. ⚠️ 各コピーに上記「この業種で効果の高いキーワード」のいずれかを含めること',
    '3. 汎用的・抽象的なコピーは禁止（例: 「今だけ限定」だけは不可、業種文脈を入れる）',
    '4. 出力はJSONのみ（文章や```は禁止）',
    '5. 推奨文字数を守る（最大40文字）',
    '6. 誇大/虚偽/規約違反の表現は避ける',
    '',
    ...
    '{ "suggestions": string[] }',
    '',
    '※ 必ず12個の「' + categoryConfig.name + '」に特化したコピーを生成してください。',
  ]
    .filter(Boolean)
    .join('\n')
