...
export default function AishodanInvitePage() {
  ...
  const accept = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/aishodan/invite/${token}`, { method: 'POST' })
      const json = await res.json()
      if (res.status === 401) {
        // 未ログインならログインへ送り、戻ってきたらこの画面に戻す
        router.push(`/auth/signin?callbackUrl=${encodeURIComponent(`/aishodan/invite/${token}`)}`)
        return
      }
      if (!res.ok) {
      ...
      setDone(true)
      setTimeout(() => router.push('/aishodan'), 1200)
    } finally {
      setBusy(false)
    }
  }, [router, token])
      ...
      <div className="w-full max-w-md rounded-lg bg-white p-8 text-center shadow-sm">
        {done ? (
          <>
            ...
            <p className="mt-2 text-sm font-semibold text-[#425071]">商談の管理画面へ移動します…</p>
          </>
        ) : error ? (
          <>
          ...
          <>
            <p className="text-sm font-black text-[#0066ff]">ドヤAI商談</p>
            ...
            <p className="mt-3 text-sm font-semibold leading-relaxed text-[#425071]">
              ...
              <br />
              参加すると、この組織の商材・商談シナリオと商談ログを扱えるようになります。
            </p>
            <button
              onClick={accept}
