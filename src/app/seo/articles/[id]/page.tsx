...
- 最終出力では「メインコピー/サブコピー」を読みやすく画像内に配置する（太字・高コントラスト）
- そのために、文字が載るエリアは「無地/ぼかし/パネル」で読みやすい背景にする（文字が潰れない）
- 背景には関係ない文字/透かし/記号を入れない（文字が読めなくなるため）
...
function SeoArticleInner() {
  ...
  function sanitizeLegacyBannerPrompt(raw: string): string {
    ...
    // 置き換えガイドを追記（既存のプロンプトの趣旨を壊さず、テキスト前提に寄せる）
    const inject = [
      '',
      '【追記（重要）】',
    ...
    // 既に同様の追記があるなら二重挿入しない
    if (out.includes('【追記（重要）】') || out.includes('最終出力ではメイン/サブ')) return out
    return `${out}\n${inject}`.trim()
  }
