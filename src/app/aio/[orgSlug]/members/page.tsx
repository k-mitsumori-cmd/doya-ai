...
import { ROLE_LABEL, type AioRole } from '@/lib/aio/types'
...
export default function AioMembersPage() {
  ...
  const [myRole, setMyRole] = useState<AioRole>('member')
  const requestSeq = useRef(0)
  const invalidateRequests = useCallback(() => { requestSeq.current++ }, [])
  const [email, setEmail] = useState('')
  const [role, setRole] = useState<AioRole>('member')
  ...
  const load = useCallback(() => {
    ...
    setMyRole('member')
    setLoadError(false)
    aioGet<{ members: Member[]; myRole: AioRole }>('/api/aio/members', orgSlug)
      .then((d) => {
  ...
  const invite = async () => {
    ...
    setInviting(true)
    try {
      const result = await aioSend<{ emailSent: boolean; inviteUrl?: string }>('/api/aio/members', orgSlug, 'POST', { email, role })
  ...
  const remove = async (m: Member) => {
    ...
    try {
      await aioSend(`/api/aio/members/${m.id}`, orgSlug, 'DELETE')
  ...
  return (
    <div className="p-6 md:p-8 max-w-3xl mx-auto">
      <PageHeader mood="thumbsup" icon="group" title="メンバー" subtitle="チームを招待して商談準備・自社情報を共有。組織ごとに情報は分離されています。" />
      ...
      {canManage && (
        <div className="rounded-3xl bg-white border border-slate-200 p-5 shadow-sm mb-6">
          ...
          <div className="flex flex-col md:flex-row gap-2">
            ...
            />
            <select value={role} onChange={(e) => setRole(e.target.value as AioRole)} className="rounded-xl border border-slate-200 px-3 py-2.5 font-bold text-sm">
          ...
          members.map((m) => (
            <div key={m.id} className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 last:border-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs font-bold text-slate-400">{ROLE_LABEL[m.role as AioRole] || m.role}</div>
              </div>
