...
export default function AioInvitePage() {
  ...
  useEffect(() => {
    fetch(`/api/aio/invite/${token}`, { cache: 'no-store' })
      .then(async (r) => {
  ...
  const accept = async () => {
    setState('accepting')
    try {
      const res = await fetch(`/api/aio/invite/${token}`, { method: 'POST' })
      const d = await res.json()
      if (res.status === 401) { signIn('google', { callbackUrl: `/aio/invite/${token}` }); return }
