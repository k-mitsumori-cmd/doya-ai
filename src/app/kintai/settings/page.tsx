...
function computeSchedulePreview(workStart: string, workEnd: string, breakMinutes: number): string {
  ...
  const midH = sh + Math.floor((totalMinutes - breakMinutes) / 2 / 60)
  const breakStart = `${String(Math.floor((sh * 60 + sm + workMinutes / 2)/ 60) | 0).padStart(2, '0')}:${String(Math.round((sh * 60 + sm + workMinutes / 2) % 60)).padStart(2, '0')}`

  // Simple approximation: assume break is in the middle
  const halfWork = Math.floor(workMinutes / 2)
  const firstHalf = halfWork
  const secondHalf = workMinutes - halfWork

  const breakStartMin = sh * 60 + sm + firstHalf
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-lg font-bold text-slate-800">就業ルール設定</h1>
      ...
      {/* Explanation */}
      <div className="flex items-start gap-2.5 bg-[#7f19e6]/5 border border-[#7f19e6]/15 rounded-xl px-4 py-3">
        <span className="material-symbols-outlined text-[#7f19e6] text-lg mt-0.5">info</span>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center py-16 text-center">
          <div className="w-20 h-20 rounded-full bg-[#7f19e6]/5 flex items-center justify-center mb-4">
            <span className="material-symbols-outlined text-5xl text-[#7f19e6]/40">rule</span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {rules.map((rule) => {
            ...
            // Calculate bar proportions for visual hours bar
            const barStart = sh * 60 + sm
            const barEnd = eh * 60 + em
            // Normalize to a 6:00-22:00 range (16 hours = 960 min) for display
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4 space-y-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
