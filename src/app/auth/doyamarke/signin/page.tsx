...
function SignInContent() {
  ...
  const shouldUseKantanLogin = useMemo(() => {
    const raw = String(callbackUrl || '').trim()
    if (!raw) return false
    try {
      if (raw.startsWith('http')) {
        const u = new URL(raw)
        if (u.pathname.startsWith('/kantan')) return true
      }
    } catch {}
    if (raw.startsWith('/kantan')) return true
    if (raw.includes('/kantan')) return true
    return false
  }, [callbackUrl])

  useEffect(() => {
    // 逆にカンタンドヤAI由来のログインでこちらに来た場合は、元のログインへ戻す
    if (typeof window === 'undefined') return
    const ref = String(document.referrer || '')
    const fromKantan = ref.includes('/kantan') || ref.includes('/auth/signin')
    if (shouldUseKantanLogin || fromKantan) {
      const qs = new URLSearchParams()
      if (callbackUrl) qs.set('callbackUrl', callbackUrl)
      if (error) qs.set('error', error)
      router.replace(`/auth/signin?${qs.toString()}`)
    }
  }, [router, shouldUseKantanLogin, callbackUrl, error])
        ...
        <div className="order-1 lg:order-2 p-7 sm:p-10 lg:p-12">
          ...
          <div className="mt-7 space-y-4">
            <button
              ...
              disabled={isLoading}
              className="w-full inline-flex items-center justify-center gap-3 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-colors px-5 py-4 font-black text-slate-800 disabled:opacity-70 min-h-[56px]"
            >
              {isLoading ? (
...
export default function DoyaMarkeSignInPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center bg-white">
