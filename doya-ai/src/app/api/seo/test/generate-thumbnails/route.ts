...
// 直接Gemini APIを呼び出して画像生成（nanobanner.tsの共通プロンプトを使用しない）
async function generateImageDirect(
  ...
  const model = process.env.DOYA_BANNER_IMAGE_MODEL || process.env.NANO_BANANA_PRO_MODEL || 'gemini-2.0-flash-exp-image-generation'
  const endpoint = `${GEMINI_API_BASE}/models/${model}:generateContent`

  // シンプルなプロンプト構成（共通テンプレートなし）
  const fullPrompt = `
${prompt}

【画像内に表示するテキスト】
メインタイトル: ${headlineText}
サブタイトル: ${subheadText}

【重要な指示】
- 上記のテキストを画像内に正確に表示してください
- 日本語テキストは正しく読める状態で表示してください
- テキストの配置位置はプロンプトの指示に従ってください
- 画像サイズ: ${width}x${height}ピクセル
- 画像全体を使用し、余白やレターボックスは入れないでください
`.trim()
  ...
  console.log(`[SEO Direct Gen] Calling Gemini API with model: ${model}`)
...
async function generateTemplateImage(
  ...
  return generateImageDirect(prompt, headlineText, subheadText)
}
