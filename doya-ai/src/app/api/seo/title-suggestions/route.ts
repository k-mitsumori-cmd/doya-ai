...
function fallbackTitles(keyword: string, count: number): string[] {
  ...
  const base = [
    `${k}とは？失敗しない選び方とチェックポイント`,
    `${k}の選び方｜比較軸と「向いている人」がすぐ分かる`,
    `${k}比較｜料金・特徴・できること/できないことを整理`,
    `【2026年版】${k}おすすめと比較ポイント`,
    `${k}で成果を出すコツ｜導入前に知っておきたい注意点`,
    `${k}導入の手順｜社内で失敗しない進め方`,
    `${k}でよくある失敗例と回避策（チェックリスト）`,
    `${k}の最新動向（2026）｜今選ぶべき理由と注目ポイント`,
  ]
...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = [
      ...
      '目的は「検索上位を狙いながら、CTR（クリック率）を最大化する」ことです。',
      '以下の条件をすべて守り、タイトル候補を指定件数だけ提案してください。',
      '',
      '【必須ルール】',
      `- 主キーワード「${primary}」を必ず含める（ただし“主キーワードだけ”で終わらせない）`,
      '- キーワードをそのまま連呼しない（語順/修飾語/具体ベネフィットで自然に含める）',
      '- 誇張しすぎない（断定・煽り・最安保証・誇大な実績の断定は禁止）',
      '- 2024年/2025年は使わない。年を入れる場合は「2026年版」または「2026」',
      '- 句読点や記号は使いすぎない（読みやすさ優先）',
      '- タイトルは可能なら28〜38文字の範囲を意識（厳密でなくてOK）',
      '- “誰の/何が得られる/何が分かる”が一瞬で伝わるようにする',
      '- 6案すべて角度を変える（比較/選び方/注意点/入門/具体例/チェックリストなど）',
      '- 6案のうち最低2案は「数字/具体性（例：チェックリスト/手順/比較軸）」を入れる',
      '',
      '【入力】',
      `- 主キーワード: ${primary}`,
      ...
      '',
      '【CTRを上げるためのヒント（必ずどれかを盛り込む）】',
      '- 読者の悩み/失敗回避/比較検討の不安を先に潰す',
      '- 「選び方」「違い」「注意点」「具体例」「テンプレ/チェックリスト」などの具体ワードを入れる',
      '- タイトルの前半に主キーワードを置けるなら置く（不自然なら無理にしない）',
      '- 競合と同じ言い回しを避け、切り口で差別化する',
      '',
      '【出力形式】',
      'JSONのみで返す。',
      `{"titles": string[] }`,
