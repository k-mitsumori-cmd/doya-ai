...
function outputBaseDir(): string {
  return process.env.DOYA_LOGO_OUTPUT_DIR?.trim() || path.join(process.cwd(), 'LOGO', 'outputs')
}
...
export async function exportProjectToDisk(project: GeneratedLogoProject): Promise<ExportResult> {
  ...
  await createZip(kitDir, zipPath)
...
async function createZip(sourceDir: string, zipPath: string) {
  ...
  const res = spawnSync('/usr/bin/zip', ['-r', zipPath, '.'], {
    cwd: sourceDir,
    encoding: 'utf8',
  })
  if (res.status !== 0) {
...
export async function exportProjectToDisk(project: GeneratedLogoProject): Promise<ExportResult> {
  ...
  // mockups (simple samples): slide / lp / business card
  const mockDir = path.join(kitDir, 'mockups')
  await fs.mkdir(mockDir, { recursive: true })
  const a = project.patterns.find((p) => p.id === 'A')
  const heroSvg = a?.logos.find((l) => l.layout === 'horizontal' && l.mode === 'default')?.svg.content || ''
